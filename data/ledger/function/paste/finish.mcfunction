...
execute if score #tmp ledger matches 1.. as @a[tag=ledger.await_result, limit=1] at @s run execute as @e[type=minecraft:villager, tag=ledger.pending, distance=..12, limit=1, sort=nearest] run tag @s add ledger.locked
